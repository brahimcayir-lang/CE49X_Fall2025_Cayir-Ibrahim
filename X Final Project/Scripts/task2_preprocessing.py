...
REPORT_FILE = os.path.join(OUTPUT_DIR, "Task2_Analysis_Report.txt")
...
def run_task2_pipeline():
    ...
    try:
        ...
        df = pd.read_sql_query("SELECT id, title, full_text FROM articles", conn)
        conn.close()
    ...
    print(f"📚 Re-Cleaning {len(df)} articles...")
    df['clean_text'] = df['full_text'].apply(preprocess_text)

    print("💾 Saving cleaned data to DB...")
    conn = sqlite3.connect(DB_PATH)
    df[['id', 'title', 'clean_text']].to_sql('articles_processed', conn, if_exists='replace', index=False)
    conn.close()
