#!/usr/bin/env python3
"""
Create stations comparison chart using the updated CSV data
"""
...
def create_stations_comparison_chart():
    """Create comparison chart for all stations"""
    
    # Read the updated CSV
    csv_file = "dsi_2000_2020_final_structured_STD_CORRECTED.csv"
    df = pd.read_csv(csv_file)
    print(f"Loaded {len(df)} records from updated dataset")
    ...
    # Save the chart
    output_path = "stations_comparison_chart_updated.png"
    ...
    summary_stats.to_csv("station_summary_statistics.csv")
    print(f"\nSummary statistics saved to: station_summary_statistics.csv")
    
    return summary_stats

def create_flow_range_chart():
    """Create a chart showing flow ranges for each station"""
    ...
    ax1.bar(x_pos, means, alpha=0.7, color='skyblue', edgecolor='navy', linewidth=1)
    ...
    bars = ax2.bar(x_pos, counts, alpha=0.7, color='lightcoral', edgecolor='darkred', linewidth=1)
    ...
    ax2.set_title('Data Points Count per Station', fontsize=14, fontweight='bold', pad=20)
    ...
    # Save the chart
    output_path = "station_flow_statistics_chart.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"\nFlow statistics chart saved to: {output_path}")
    
    plt.show()

if __name__ == "__main__":
    print("Creating stations comparison chart with updated data...")
    
    # Create main comparison chart
    summary_stats = create_stations_comparison_chart()
    
    # Create flow statistics chart
    create_flow_range_chart()
