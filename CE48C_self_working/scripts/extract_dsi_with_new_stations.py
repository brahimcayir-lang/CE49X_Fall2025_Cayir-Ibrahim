#!/usr/bin/env python3
"""
DSİ PDF Complete Data Extractor - Flexible version for adding new stations
"""
...
# Configure logging
logging.basicConfig(level=logging.INFO, format='%(asktime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

class DSIExtractorComplete:
    """Complete DSİ PDF data extractor that handles index-to-data mapping correctly."""
    
    def __init__(self, pdf_directory: str, output_csv: str = "dsi_complete_data.csv", 
                 target_stations: set = None):
        self.pdf_directory = Path(pdf_directory)
        self.output_csv = output_csv
        
        # Default target station codes to extract
        default_stations = {
            'D22A144', 'D22A145', 'D22A065', 'D22A093', 
            'D22A095', 'D14A149', 'D22A158'
        }
        
        # Use provided stations or default
        self.target_stations = target_stations if target_stations else default_stations
        
        logger.info(f"Target stations: {sorted(self.target_stations)}")
...
def main():
    """Main function with example of adding new stations."""
    ...
    new_stations = {
        ...
        'D14A162', 'D22A105', 'D22A192', 'D22A172'  # Example new stations
        # Add more stations as needed:
        # 'D22A200', 'D22A201', 'D22A202'  # Your new station codes
    }
    
    # Configuration
    PDF_DIRECTORY = r"C:\Users\Asus\Desktop\bitirme_projesi\debi_raporlari\akim_gözlem_yilligi"
    OUTPUT_CSV = "dsi_complete_data_with_new_stations.csv"
    
    # Create extractor with new stations
