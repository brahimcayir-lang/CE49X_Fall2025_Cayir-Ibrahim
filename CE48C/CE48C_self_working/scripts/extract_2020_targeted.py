#!/usr/bin/env python3
"""
Targeted 2020 DSİ data extraction based on exact screenshot format
"""
...
def extract_2020_data_targeted():
    """Extract 2020 data with targeted parsing for exact format"""
    ...
    target_stations = {
        ...
        'D14A172', 'D14A192', 'D14A018', 'D22A093', 'D22A095', 'D22A105',r
    ...
    print("Extracting 2020 data with TARGETED parsing...")
    ...
    for page_num in range(len(doc)):
        ...
        for station_code in target_stations:
            if station_code in text:
                ...
                station_data = extract_station_data_targeted(text, station_code, page_num + 1)
                if station_data:
...
def extract_station_data_targeted(text, station_code, page_num):
    """Extract data with targeted parsing for exact format"""
    lines = text.split('\n')
    ...
    # Look for annual total from footer - "SU YILI ( 2020 ) YILLIK TOPLAM AKIM ... MİLYON M3 ... MM. ... LT/SN/Km2"
    annual_total = None
    mm_total = None
    avg_ltsnkm2 = None
    ...
    for i, line in enumerate(lines):
        if 'SU YILI' in line and 'YILLIK TOPLAM AKIM' in line and 'MİLYON M3' in line:
            # Extract numbers from footer line
            numbers = re.findall(r'(\d+[.,]\d+|\d+)', line)
            if len(numbers) >= 3:
                annual_total = float(numbers[0].replace(',', '.'))  # MİLYON M3
                mm_total = float(numbers[1].replace(',', '.'))      # MM
                avg_ltsnkm2 = float(numbers[2].replace(',', '.'))    # LT/SN/Km2
            break
    
    # Look for monthly data - the 6 metric blocks at the bottom
    monthly_data = extract_monthly_data_targeted(text)
...
def extract_monthly_data_targeted(text):
...
def update_csv_with_targeted_2020_data():
    """Update CSV with TARGETED 2020 data"""
    csv_path = r"C:\Users\Asus\Desktop\bitirme_projesi\outputs\dsi_2000_2020_final_structured.csv"
    
    # Extract targeted data
    extracted_data = extract_2020_data_targeted()
    ...
    print(f"\nUpdated CSV with {len(extracted_data)} TARGETED 2020 records")
    ...
    # Check D22A095 specifically
    d22a095_data = [d for d in extracted_data if d['station_code'] == 'D22A095']
    if d22a095_data:
        data = d22a095_data[0]
        print(f"\nD22A095 specific data:")
...
def main():
    update_csv_with_targeted_2020_data()
