#!/usr/bin/env python3
"""
FINAL CORRECTED 2020 DSİ data extraction - properly replace all 2020 data
"""
...
def extract_2020_data_final():
    """Extract 2020 data with FINAL corrected parsing"""
    ...
    print("Extracting 2020 data with FINAL corrected parsing...")
    ...
    for page_num in range(len(doc)):
        ...
        for station_code in target_stations:
            if station_code in text:
                ...
                station_data = extract_station_data_final(text, station_code, page_num + 1)
                if station_data:
...
def extract_station_data_final(text, station_code, page_num):
    """Extract data with FINAL corrected parsing"""
    lines = text.split('\n')
    ...
    # FINAL CORRECTED: Look for annual total from footer
    annual_total = None
    mm_total = None
    avg_ltsnkm2 = None
    ...
    monthly_data = extract_monthly_data_final(text)
...
def extract_monthly_data_final(text):
...
def update_csv_with_final_2020_data():
    """Update CSV with FINAL corrected 2020 data - properly replace all 2020 data"""
    csv_path = r"C:\Users\Asus\Desktop\bitirme_projesi\outputs\dsi_2000_2020_final_structured.csv"
    
    # Extract final corrected data
    extracted_data = extract_2020_data_final()
    ...
    # COMPLETELY REMOVE all 2020 data
    existing_df = existing_df[existing_df['year'] != 2020]
    print(f"Removed all 2020 data. Remaining records: {len(existing_df)}")
    ...
    print(f"\nFINAL UPDATE: Added {len(extracted_data)} CORRECTED 2020 records")
...
def main():
    update_csv_with_final_2020_data()
