#!/usr/bin/env python3
"""
Fixed 2020 DSİ hydrological data extraction based on actual PDF format
"""
...
def extract_2020_data_fixed():
    """Extract 2020 data with correct parsing based on actual PDF format"""
    ...
    print("Extracting 2020 data with FIXED parsing based on actual PDF format...")
    ...
    for page_num in range(len(doc)):
        ...
        for station_code in target_stations:
            if station_code in text:
                ...
                station_data = extract_station_data_fixed(text, station_code, page_num + 1)
                if station_data:
...
def extract_station_data_fixed(text, station_code, page_num):
    """Extract data for a specific station with FIXED parsing"""
    lines = text.split('\n')
    ...
    for i in range(max(0, station_line_idx - 10), min(len(lines), station_line_idx + 15)):
        line = lines[i]
        # Look for patterns like "2020 Su yılında ... m3/sn"
        if 'Su yılında' in line and ('m3/sn' in line or 'm³/s' in line or 'm3/s' in line):
    ...
    # Look for annual total from footer
    annual_total = None
    mm_total = None
    avg_ltsnkm2 = None
    ...
    monthly_data = extract_monthly_data_fixed(text)
...
def extract_monthly_data_fixed(text):
...
def update_csv_with_fixed_2020_data():
    """Update CSV with FIXED 2020 data"""
    csv_path = r"C:\Users\Asus\Desktop\bitirme_projesi\outputs\dsi_2000_2020_final_structured.csv"
    
    # Extract fixed data
    extracted_data = extract_2020_data_fixed()
    ...
    print(f"\nUpdated CSV with {len(extracted_data)} FIXED 2020 records")
...
def main():
    update_csv_with_fixed_2020_data()
