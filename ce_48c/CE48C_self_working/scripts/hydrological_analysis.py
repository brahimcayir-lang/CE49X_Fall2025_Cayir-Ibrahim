...
def create_annual_trend_analysis(df):
    ...
    ax1.plot(df_sorted['year'], df_sorted['annual_total_m3_million'], 
             'o-', alpha=0.7, linewidth=2, markersize=6, label='Annual Volume')
    
    # Rolling mean (5-year window)
    rolling_mean = df_sorted['annual_total_m3_million'].rolling(window=5, center=True).mean()
    ax1.plot(df_sorted['year'], rolling_mean, 
             'r-', linewidth=3, label='5-Year Rolling Mean')
    ...
    ax2.plot(df_sorted['year'], df_sorted['annual_lps_km2'], 
             'o-', alpha=0.7, linewidth=2, markersize=6, label='Specific Discharge', color='green')
    
    rolling_mean_spec = df_sorted['annual_lps_km2'].rolling(window=5, center=True).mean()
    ax2.plot(df_sorted['year'], rolling_mean_spec, 
             'r-', linewidth=3, label='5-Year Rolling Mean')
    ...
    slope_vol, intercept_vol, r_value_vol, p_value_vol, std_err_vol = stats.linregress(df_sorted['year'], df_sorted['annual_total_m3_million'])
    slope_spec, intercept_spec, r_value_spec, p_value_spec, std_err_spec = stats.linregress(df_sorted['year'], df_sorted['annual_lps_km2'])
...
def create_monthly_regime_analysis(df):
    ...
    for month in months:
        monthly_data.extend(df[month].dropna().tolist())
    
    # Create monthly DataFrame
    monthly_df = pd.DataFrame({
        'Month': months * len(df),
        'Discharge': [df[month].iloc[i] for month in months for i in range(len(df)) if pd.notna(df[month].iloc[i])]
    })
    ...
    ax1.plot(range(12), monthly_means, 'o-', linewidth=3, markersize=8, color='darkblue')
    ax1.fill_between(range(12), monthly_means - monthly_std, monthly_means + monthly_std, 
    ...
    monthly_data_for_box = [df[month].dropna().tolist() for month in months]
    ...
    wet_avg = df[wet_season].mean(axis=1).mean()
    dry_avg = df[dry_season].mean(axis=1).mean()
...
def create_flow_duration_curve(df):
    ...
    for month in months:
        all_flows.extend(df[month].dropna().tolist())
...
def create_correlation_analysis(df):
    ...
    # Scatter plot
    plt.scatter(df['annual_lps_km2'], df['annual_total_m3_million'], 
               s=100, alpha=0.7, c=df['year'], cmap='viridis', edgecolors='black')
    ...
    # Add trend line
    slope, intercept, r_value, p_value, std_err = stats.linregress(df['annual_lps_km2'], df['annual_total_m3_million'])
    line = slope * df['annual_lps_km2'] + intercept
    plt.plot(df['annual_lps_km2'], line, 'r--', linewidth=2, 
...
def create_extreme_flow_analysis(df):
    ...
    annual_max = df[['Ekim', 'Kasım', 'Aralık', 'Ocak', 'Şubat', 'Mart', 
                    'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül']].max(axis=1)
    
    ax1.plot(df['year'], annual_max, 'o-', linewidth=2, markersize=6, color='red', label='Annual Maximum')
    ax1.fill_between(df['year'], annual_max, alpha=0.3, color='red')
    ...
    annual_min = df[['Ekim', 'Kasım', 'Aralık', 'Ocak', 'Şubat', 'Mart', 
                    'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül']].min(axis=1)
    
    ax2.plot(df['year'], annual_min, 'o-', linewidth=2, markersize=6, color='blue', label='Annual Minimum')
    ax2.fill_between(df['year'], annual_min, alpha=0.3, color='blue')
    ...
    max_flow = annual_max.max()
    min_flow = annual_min.min()
    avg_max = annual_max.mean()
    avg_min = annual_min.mean()
...
def generate_hydropower_insights(df):
    ...
    q50 = np.percentile(df[['Ekim', 'Kasım', 'Aralık', 'Ocak', 'Şubat', 'Mart', 
                           'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül']].values.flatten(), 50)
    q90 = np.percentile(df[['Ekim', 'Kasım', 'Aralık', 'Ocak', 'Şubat', 'Mart', 
                           'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül']].values.flatten(), 10)
    ...
    wet_avg = df[wet_months].mean(axis=1).mean()
    dry_avg = df[dry_months].mean(axis=1).mean()
    ...
    q10 = np.percentile(df[['Ekim', 'Kasım', 'Aralık', 'Ocak', 'Şubat', 'Mart', 
                           'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül']].values.flatten(), 90)
    max_flow = df[['Ekim', 'Kasım', 'Aralık', 'Ocak', 'Şubat', 'Mart', 
                   'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül']].max().max()
