#!/usr/bin/env python3
"""
Create stations comparison chart with overall trend (no error bars displayed)
"""
...
def create_stations_comparison_with_overall_trend():
    """Create comparison chart with overall trend line (no error bars)"""
    ...
    # Calculate overall yearly averages (including std dev for background analysis)
    ...
    # Add text box with overall statistics (including std dev info)
...
Avg Yearly Std Dev: {yearly_avg['std'].mean():.3f} m³/s"""
    ...
    # Save the chart
    output_path = "stations_comparison_with_overall_trend_clean.png"
    ...
    summary_stats.to_csv("station_summary_with_overall_trend_clean.csv")
    print(f"\nSummary statistics saved to: station_summary_with_overall_trend_clean.csv")
    
    return summary_stats, yearly_avg

def create_overall_trend_analysis_chart(yearly_avg, df_flow):
    """Create a focused chart showing the overall trend analysis (no error bars)"""
    
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(16, 12))
    
    # Chart 1: Overall trend (clean, no error bars)
    ...
    # Save the chart
    output_path = "overall_trend_analysis_clean.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight', facecolor='white')
...
if __name__ == "__main__":
    print("Creating stations comparison chart with overall trend (no error bars)...")
    ...
    print("Charts show individual station patterns and overall average trend.")
    print("Standard deviation calculated in background but not displayed on charts.")
