#!/usr/bin/env python3
"""
Improved Turkish-Aware DSİ Hydrological Station Data Extractor
Fixed table parsing to properly extract all 12 months of data
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('dsi_extraction_improved.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class ImprovedTurkishDSIExtractor:
    def __init__(self, pdf_directory: str, output_file: str):
        ...
        # Turkish metric keywords - more comprehensive
        self.metric_keywords = {
            'flow_max': ['Maks.', 'MAKS.', 'maksimum', 'Maksimum'],
            'flow_min': ['Min.', 'MIN.', 'minimum', 'Minimum'],
            'flow_avg': ['Ortalama', 'ORTALAMA', 'ortalama', 'Ortalama'],
            'ltsnkm2': ['LT/SN/Km2', 'LT/SN/KM2', 'lt/sn/km2', 'LT/SN/Km²'],
            'akim_mm': ['AKIM mm.', 'AKIM MM.', 'akim mm', 'AKIM mm'],
            'milm3': ['MİL. M3', 'MIL. M3', 'mil. m3', 'MİL. M³']
        }
    ...
    def _extract_monthly_data_improved(self, text: str) -> Dict[str, Dict[str, float]]:
        """Improved monthly data extraction with better table parsing"""
        monthly_data = {}
        
        # Split text into lines
        lines = text.split('\n')
        
        # Look for the monthly data table - it should contain month names
        table_start = -1
        turkish_months = ["Ekim", "Kasım", "Aralık", "Ocak", "Şubat", "Mart",
                         "Nisan", "Mayıs", "Haziran", "Temmuz", "Ağustos", "Eylül"]
        
        # Find lines that contain month names
        month_lines = []
        for i, line in enumerate(lines):
            if any(month in line for month in turkish_months):
                month_lines.append((i, line))
        
        if not month_lines:
            logger.warning("No month names found in text")
            return monthly_data
        ...
        # Extract data from table rows - look for lines with metric keywords
        for i in range(table_start, min(table_start + 10, len(lines))):  # Check more lines
            line = lines[i]
            ...
            # Extract numbers from this line - look for 12 values
            numbers = re.findall(r'[\d.,]+', line)
            
            if len(numbers) >= 6:  # At least 6 values (some tables might have fewer months)
                logger.info(f"Found {len(numbers)} values for {current_metric}")
                
                # Map Turkish months to English - use the order found in the text
                # Try to find the month order from the text
                month_order = []
                for month_line_idx, month_line in month_lines:
                    if month_line_idx > table_start and month_line_idx < i + 10:
                        for month in turkish_months:
                            if month in month_line and month not in month_order:
                                month_order.append(month)
                                break
                
                # If we couldn't determine order, use default chronological order
                if not month_order:
                    month_order = turkish_months
                
                # Map values to months
                for j, turkish_month in enumerate(month_order):
                    if j < len(numbers):
                ...
                logger.warning(f"Only found {len(numbers)} values for {current_metric}, expected 6-12")
    ...
    def _extract_station_data_improved(self, page_text: str, page_num: int, filename: str, year: int) -> Optional[Dict]:
        """Extract all station data with improved Turkish character support"""
        ...
        monthly_data = self._extract_monthly_data_improved(page_text)
    ...
    def _process_pdf_improved(self, pdf_path: str) -> List[Dict]:
        """Process a single PDF file with improved Turkish character support"""
        results = []
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                station_data = self._extract_station_data_improved(page_text, page_num + 1, filename, year)
                if station_data:
    ...
    def process_all_pdfs_improved(self):
        """Process all PDF files with improved Turkish character support"""
        ...
        for pdf_file in pdf_files:
            ...
            # Process PDF
            results = self._process_pdf_improved(pdf_path)
...
def main():
    ...
    output_file = r"C:\Users\Asus\Desktop\bitirme_projesi\outputs\dsi_2000_2020_monthly_full_improved.csv"
    
    # Create output directory if it doesn't exist
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    
    # Initialize extractor
    extractor = ImprovedTurkishDSIExtractor(pdf_directory, output_file)
    
    # Process all PDFs
    extractor.process_all_pdfs_improved()
